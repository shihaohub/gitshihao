...
Top::Top(ros::NodeHandle &nh, ros::NodeHandle &nh_priv) {
  ...
  nh_priv.param<double>("GLOBAL_ZERO_Z_", GLOBAL_ZERO_Z_, 12);
  nh_priv.param<int>("utm_origin_band", GLOBAL_ZERO_Band_, 82);
  nh_priv.param<int>("utm_origin_zone", GLOBAL_ZERO_Zone_, 49);
  
  // utm2gps
  geodesy::UTMPoint utmPoint;
  utmPoint.band = GLOBAL_ZERO_Band_;
  utmPoint.zone = GLOBAL_ZERO_Zone_;
  utmPoint.easting = GLOBAL_ZERO_X_;
  utmPoint.northing = GLOBAL_ZERO_Y_;
  utmPoint.altitude = GLOBAL_ZERO_Z_;
  geographic_msgs::GeoPoint geopoint=geodesy::toMsg(utmPoint);
  GLOBAL_ZERO_Longitude_ = geopoint.longitude;
  GLOBAL_ZERO_Latitude_ = geopoint.latitude;
  GLOBAL_ZERO_Altitude_ = geopoint.altitude;

  EARTH_RAD_EQ_ = 1000 * 6378.137;
  SCALE_ = cos(GLOBAL_ZERO_Latitude_* M_PI / 180.0);
  OFFSET_X_ = SCALE_ * EARTH_RAD_EQ_ * GLOBAL_ZERO_Longitude_ * M_PI / 180.0;
  OFFSET_Y_ = SCALE_ * EARTH_RAD_EQ_ * log(tan((90.0 + GLOBAL_ZERO_Latitude_) * (M_PI / 360.0)));
...
void Top::gps_callback(const sensor_msgs::NavSatFixConstPtr &gps_in,
  ...
  MapFrame obs_pos(EARTH_RAD_EQ_, OFFSET_X_, OFFSET_Y_, SCALE_);
  ...
  } else if (status_in->status == 4 && status_in->num_satellites > 5) {
    if (gps_warning_count < 100) gps_warning_count++;
    if (gps_error_count > 0) gps_error_count--;
  } else {
    if (gps_warning_count < 100) gps_warning_count++;
    if (gps_error_count < 1000) gps_error_count++;
  }
...
void Top::filter_callback(const ros::TimerEvent &) {
  ...
  MapFrame output_pos(EARTH_RAD_EQ_, OFFSET_X_, OFFSET_Y_, SCALE_, X(0), X(1));
