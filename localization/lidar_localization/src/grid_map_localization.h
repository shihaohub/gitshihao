...
enum LocalizationStatus { WAITING, INIT, MATCHING };

enum InitializationMode { AUTO = 0, MANUAL };
...
public:
    ...
    inline void SetGridMapInfo(int grid_map_size,
        ...
        grid_map_.InitGridMap(
          grid_map_size, grid_map_resolution, grid_map_folder_path);
    }
    ...
    inline const double GetMatchingGoodness() {
        return matcher_.GetIcpGoodness();
    }

    inline const double GetMatchingQuality() {
        return matcher_.GetIcpQuality();
    }

    inline void
    SetMatchingInitPose(const double &x, const double &y, double &angle) {
        if (matching_init_ == nullptr) matching_init_ = new Pose2DStamped;
    ...
    inline void
    SetMatchingInitPose(const localization::StateEstimation poseture) {
        if (matching_init_ == nullptr) matching_init_ = new Pose2DStamped;
    ...
    inline void SetIcpParameters(int maxIterations){
        matcher_.SetIcpMaxIterations(maxIterations);
    }
