...
// fixedpose
std::vector<localization::Pose2D> fixed_pose;
int fixed_pose_num = 0;
// utm参数
...
bool indoor_fixed_pose_flag = false;
int delay_num = 0;
int delay_num_ = 0;
int stay_count = 0;
...
std::string indoor_init_cmd_topic;

ros::Publisher *init_status_publisher;
...
void IndoorInitCmdCallback(const std_msgs::UInt8 &indoor_init_cmd_msg);

void PointCloudPublishingCallback(const ros::TimerEvent &);

void MapPublishingCallback(const ros::TimerEvent &);

void InitStatusPublishingCallback(const ros::TimerEvent &);
...
int main(int argc, char **argv) {
  printf("LidarLocalization module version: 1.2.2\n");
  ...
  pnh.param<std::string>("imu_topic", imu_topic, "/imu/angular_velocity");
  pnh.param<std::string>("laser_topic", laser_topic,
                         "/driver/livox/point_cloud");
  pnh.param<std::string>("gps_status_topic", gps_status_topic,
                         "/strong/raw_data");
  pnh.param<std::string>("indoor_init_cmd_topic", indoor_init_cmd_topic,
                         "/lidar_localization_node/indoor_init_cmd");
  //起始方式
  ...
  // ICP配准参数
  pnh.param<int>("icp_maxIterations", icp_maxIterations, 25);
  // fixedpose
  if (initialization_mode == 2) {
    pnh.param("fixed_pose/fixed_pose_num", fixed_pose_num, fixed_pose_num);
    fixed_pose.resize(fixed_pose_num);
    for (int i = 0; i < fixed_pose_num; i++) {
      pnh.param("fixed_pose/fixed_pose_x" + std::to_string(i), fixed_pose[i].x,
                fixed_pose[i].x);
      pnh.param("fixed_pose/fixed_pose_y" + std::to_string(i), fixed_pose[i].y,
                fixed_pose[i].y);
      pnh.param("fixed_pose/fixed_pose_yaw" + std::to_string(i),
                fixed_pose[i].phi, fixed_pose[i].phi);
    }
  }
  ...
  // // 经纬度原点设定
  // pnh.param("GLOBAL_ZERO_Longitude_", gps_origin_longitude,
  // gps_origin_longitude); pnh.param("GLOBAL_ZERO_Latitude_",
  // gps_origin_latitude, gps_origin_latitude);
  // pnh.param("GLOBAL_ZERO_Altitude_", gps_origin_altitude,
  // gps_origin_altitude);

  // // utm2gps
  // geodesy::UTMPoint utmPoint;
  // utmPoint.band = utm_band;
  // utmPoint.zone = utm_zone;
  // utmPoint.easting = utm_origin_x;
  // utmPoint.northing = utm_origin_y;
  // utmPoint.altitude = utm_origin_z;
  // geographic_msgs::GeoPoint geopoint=geodesy::toMsg(utmPoint);
  // sensor_msgs::NavSatFix fix_out;
  // fix_out.longitude = geopoint.longitude;
  // fix_out.latitude = geopoint.latitude;
  // fix_out.altitude = geopoint.altitude;
  // std::cout<< std::fixed << std::setprecision(8)<<"longitude is
  // "<<fix_out.longitude<<std::endl; std::cout<<"latitude is
  // "<<fix_out.latitude<<std::endl; std::cout<<"altitude is
  // "<<fix_out.altitude<<std::endl;

  // // gps2utm
  // geographic_msgs::GeoPointStampedPtr gps_msg(new
  // geographic_msgs::GeoPointStamped()); gps_msg->position.longitude =
  // gps_origin_longitude; gps_msg->position.latitude = gps_origin_latitude;
  // gps_msg->position.altitude = gps_origin_altitude;
  // geodesy::UTMPoint utm_point;
  // geodesy::fromMsg(gps_msg->position, utm_point);
  // utm_origin_x = utm_point.easting;
  // utm_origin_y = utm_point.northing;
  // utm_origin_z = utm_point.altitude;
  // utm_band = utm_point.band;
  // utm_zone = utm_point.zone;
  // std::cout<< std::fixed << std::setprecision(8)<<"x is
  // "<<utm_origin_x<<std::endl; std::cout<<"y is "<<utm_origin_y<<std::endl;
  // std::cout<<"z is "<<utm_origin_z<<std::endl;
  // std::cout<<"band is "<<utm_band<<std::endl;
  // std::cout<<"zone is "<<utm_zone<<std::endl;

  //点云截取范围
  pnh.param("cloud_boudingbox/cloud_boundingbox_max_x", cloud_boundingbox.max.x,
            cloud_boundingbox.max.x);
  pnh.param("cloud_boudingbox/cloud_boundingbox_min_x", cloud_boundingbox.min.x,
            cloud_boundingbox.min.x);
  pnh.param("cloud_boudingbox/cloud_boundingbox_max_y", cloud_boundingbox.max.y,
            cloud_boundingbox.max.y);
  pnh.param("cloud_boudingbox/cloud_boundingbox_min_y", cloud_boundingbox.min.y,
            cloud_boundingbox.min.y);
  pnh.param("cloud_boudingbox/cloud_boundingbox_max_z", cloud_boundingbox.max.z,
            cloud_boundingbox.max.z);
  pnh.param("cloud_boudingbox/cloud_boundingbox_min_z", cloud_boundingbox.min.z,
            cloud_boundingbox.min.z);

  pnh.param("has_CL", has_CL, has_CL);
  pnh.param("loc_cloud_boudingbox/loc_cloud_boundingbox_max_x",
            loc_cloud_boundingbox.max.x, loc_cloud_boundingbox.max.x);
  pnh.param("loc_cloud_boudingbox/loc_cloud_boundingbox_min_x",
            loc_cloud_boundingbox.min.x, loc_cloud_boundingbox.min.x);
  pnh.param("loc_cloud_boudingbox/loc_cloud_boundingbox_max_y",
            loc_cloud_boundingbox.max.y, loc_cloud_boundingbox.max.y);
  pnh.param("loc_cloud_boudingbox/loc_cloud_boundingbox_min_y",
            loc_cloud_boundingbox.min.y, loc_cloud_boundingbox.min.y);
  pnh.param("loc_cloud_boudingbox/loc_cloud_boundingbox_max_z",
            loc_cloud_boundingbox.max.z, loc_cloud_boundingbox.max.z);
  pnh.param("loc_cloud_boudingbox/loc_cloud_boundingbox_min_z",
            loc_cloud_boundingbox.min.z, loc_cloud_boundingbox.min.z);

  //激光雷达和车辆中心的标定值
  pnh.param("lidar_to_vehicle/lidar_to_vehicle_x", lidar_cali_info.x,
            lidar_cali_info.x);
  pnh.param("lidar_to_vehicle/lidar_to_vehicle_y", lidar_cali_info.y,
            lidar_cali_info.y);
  pnh.param("lidar_to_vehicle/lidar_to_vehicle_z", lidar_cali_info.z,
            lidar_cali_info.z);
  pnh.param("lidar_to_vehicle/lidar_to_vehicle_rx", lidar_cali_info.rx,
            lidar_cali_info.rx);
  pnh.param("lidar_to_vehicle/lidar_to_vehicle_ry", lidar_cali_info.ry,
            lidar_cali_info.ry);
  pnh.param("lidar_to_vehicle/lidar_to_vehicle_rz", lidar_cali_info.rz,
            lidar_cali_info.rz);
  //激光雷达和车辆中心的标定值
  pnh.param("gps_to_vehicle/gps_to_vehicle_x", gps_cali_info.x,
            gps_cali_info.x);
  pnh.param("gps_to_vehicle/gps_to_vehicle_y", gps_cali_info.y,
            gps_cali_info.y);
  //降采样值
  pnh.param("voxelgrid_filter_size", voxelgrid_filter_size,
            voxelgrid_filter_size);
  ...
  pnh.param<std::string>("data_folder_path", data_folder_path, "./data");
  // slam运行区域
  ...
  gml->SetIcpParameters(icp_maxIterations);
  ...
  ros::Subscriber gps_status_subscriber =
      nh.subscribe(gps_status_topic, 1, StrongStatusCallback);
  ros::Subscriber speed_subscriber =
      nh.subscribe(speed_topic, 1, SpeedCallback);
  ros::Subscriber imu_subscriber = nh.subscribe(imu_topic, 1, ImuCallback);
  ros::Subscriber pointcloud_subscriber =
      nh.subscribe(laser_topic, 10, PointCloudCallback);
  ros::Subscriber indoor_init_cmd_subscriber =
      nh.subscribe(indoor_init_cmd_topic, 1, IndoorInitCmdCallback);

  init_status_publisher =
      new ros::Publisher(pnh.advertise<std_msgs::UInt8>("init_status", 1));
  map_publisher =
  ...
  ros::Timer init_status_publishing_timer =
      nh.createTimer(ros::Duration(0.05), InitStatusPublishingCallback);
  ...
  if (initialization_mode == 1) {
    ...
    std::cout << "waiting for indoor fixed pose parking..." << std::endl;
  }
...
void SpeedCallback(const cyber_msgs::VehicleSpeedFeedbackConstPtr &vel_msg) {
  ...
  if (std::isnan(speed)) ROS_ERROR("speed_error");
  localization::util::CalStaEsTwi(&global_posture, speed);
  speed_updated_flag = true;
}

void ImuCallback(const geometry_msgs::Vector3StampedConstPtr &imu_msg) {
  curr_yaw_rate = imu_msg->vector.z;
  double now_time = imu_msg->header.stamp.toSec();
  if (init_flag && slam_area_flag) {
    if ((last_imu_time != -1) && abs(curr_yaw_rate) > 0.04) {
      cps_yaw += curr_yaw_rate * (now_time - last_imu_time);
    }
  }
  last_imu_time = now_time;
  // ROS_INFO("cps_yaw from imu: %f", cps_yaw);
}
...
void PointCloudCallback(const sensor_msgs::PointCloud2ConstPtr &cloud_msg) {
  ...
  //河西17室内初始化
  if ((!init_flag) && (initialization_mode == 2) && indoor_fixed_pose_flag) {
    std::vector<double> fixed_pose_quality(fixed_pose_num);

    for (int i = 0; i < fixed_pose_num; i++) {
      gml->LoadGridMap(fixed_pose[i].x, fixed_pose[i].y);
      global_posture.pose.position.x = fixed_pose[i].x;
      global_posture.pose.position.y = fixed_pose[i].y;
      init_pose.x = fixed_pose[i].x;
      init_pose.y = fixed_pose[i].y;
      init_pose.phi = fixed_pose[i].phi;
      gml->UpdatePointCloudPose(init_pose);
      gml->SetMatchingInitPose(init_pose.x, init_pose.y,
                               init_pose.phi);  //设置匹配起始位置
      for (int j = 0; j < 10; j++) {
        gml->PointCloudMatch();
        fixed_pose_quality[i] += gml->GetMatchingQuality();
      }
      fixed_pose_quality[i] = fixed_pose_quality[i] / 10.0;
      std::cout << "fixed_pose_quality[" << i
                << "] is:" << fixed_pose_quality[i] << '\n';
    }

    int i = max_element(fixed_pose_quality.begin(), fixed_pose_quality.end()) -
            fixed_pose_quality.begin();
    {
      gml->LoadGridMap(fixed_pose[i].x, fixed_pose[i].y);
      global_posture.pose.position.x = fixed_pose[i].x;
      global_posture.pose.position.y = fixed_pose[i].y;
      ...
      gml->SetMatchingInitPose(init_pose.x, init_pose.y,
                               init_pose.phi);  //设置匹配起始位置
      std::cout << "[OK] init slam by fixed pose " + std::to_string(i) << '\n';
      slam_area_flag = true;
      delay_num = 0;
      init_flag = true;
    }
  ...
  if (((initialization_mode != 2) && init_flag && gps_fix_updated_flag &&
       heading_updated_flag && slam_area_flag) ||
      (initialization_mode == 2) && init_flag && slam_area_flag) {
    //速度补偿
    if (pose_update && (delay_num == 0) && last_point_time != -1) {
      double temp_x = 0;
      double temp_y = 0;
      double temp_angle = 0;
      double temp_dis =
          speed * (cloud_msg->header.stamp.toSec() - last_point_time);
      temp_x = last_pose.x + temp_dis * cos(gloabl_angle);
      temp_y = last_pose.y + temp_dis * sin(gloabl_angle);
      temp_angle = last_pose.phi + cps_yaw;
      cps_yaw = 0;
      gml->SetMatchingInitPose(temp_x, temp_y, temp_angle);
      pose_update = false;
    }
    last_point_time = cloud_msg->header.stamp.toSec();
    gml->PointCloudMatch();
    ...
    if (localization_result != nullptr) {
      ...
      //长廊效应临时解决措施
      //    if(gps_status == 4){
      //     double diff_x = gps_posture.pose.position.x -
      //     localization_result->pose.x; double diff_y =
      //     gps_posture.pose.position.y - localization_result->pose.y;
      //     double heading_diff =
      //     cos(localization_result->pose.phi)*diff_x +
      //     sin(localization_result->pose.phi)*diff_y;
      ...
      last_pose.x = global_posture.pose.position.x;
      last_pose.y = global_posture.pose.position.y;
      last_pose.phi = localization_result->pose.phi;
 ...
 * @author: Ying Wu
 * @description:
 * 室内初始化开始标志位加载，说明车已开到固定点位置，需要开始初始化
 * @date: 2022-04-21 19:22:34
 * @param {UInt8} &indoor_init_cmd_msg
 * @return {*}
 */
void IndoorInitCmdCallback(const std_msgs::UInt8 &indoor_init_cmd_msg) {
  if ((initialization_mode == 2) && !init_flag && indoor_init_cmd_msg.data) {
    std::cout << "waiting for indoor init..." << std::endl;
    indoor_fixed_pose_flag = 1;
  }
}
...
/**
 * @author: Ying Wu
 * @description: 初始化状态发布，0初始化未完成，1初始化完成，兼容三种初始化模式
 * @date: 2022-04-21 13:15:15
 * @param {TimerEvent} &
 * @return {*}
 */
void InitStatusPublishingCallback(const ros::TimerEvent &) {
  std_msgs::UInt8 init_status;
  if (init_flag)
    init_status.data = 1;
  else
    init_status.data = 0;
  init_status_publisher->publish(init_status);
}
